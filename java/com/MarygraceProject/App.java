...
public class App {
	...
	public static void main(String[] args) {
		...
		Marygrace secondMarge = new Marygrace(pets, bestPersonName);
		// our second marge using other constructor
		// make sure names are in second order

		System.out.println("Your two Marge's have been created");
		// print line for java
		// calls system class
		...
		boolean doMarygraceObjectsHaveSamePetNum = MarygraceUtil.compareMarygraceObjectsHaveSamePetNumber(firstMarge,
				secondMarge);
		if (doMarygraceObjectsHaveSamePetNum) {
			...
			//should be here
			System.out.println("Marge1 has " + firstMarge.getNumberOfPets() + " while Marge2 has "
					+ secondMarge.getNumberOfPets() + " pets");
		}
		...
		// please note that this is not pass by reference and is technically pass by value. The
		// objects themselves are
		// are pointers to specified objects in main. We can modify their values but not
		// where they point
		// For this method, a new firstMarge gets created inside the method call,
		// referenced by the one
		// in this method.

		// So for simple data types like int, if i were to pass 2 integer values to
		// swap, the referenced
		// values inside main would never get swapped. We cannot change where simple
		// datatypes
		...
		// however i can change their data fields.

		// this is confusing i get it but for simplicity sake, you can modify objects in
		// methods, but
		// not where they point to. For simple types like, char and int, you cannot
		// modify their
